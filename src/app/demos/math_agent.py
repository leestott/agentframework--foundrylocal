...
The agent can:
  ...
  • Solve unit conversions
  • Show calculation steps

This showcases how an agent can use tools for precise calculations
instead of relying on the LLM's (often unreliable) math abilities.
"""
...
def convert_units(
    ...
    if key not in conversions:
        available = ", ".join(f"{f}→{t}" for f, t in conversions.keys())
...
register_demo(DemoInfo(
    ...
    description="Precise calculation agent with tools for arithmetic, percentages, unit conversions, compound interest, and statistics. The agent uses tools instead of mental math for accuracy.",
    icon="🔢",
