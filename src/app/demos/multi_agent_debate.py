...
This creates two agents with different perspectives who debate a topic:
  • Agent A: Argues FOR the proposition
  • Agent B: Argues AGAINST the proposition
  • Moderator: Summarizes key points and declares a winner

This showcases:
  • Multiple agents in a single workflow
  • Sequential orchestration with dependencies
  • Agents receiving output from other agents
  • Emergent reasoning from agent interaction
"""
...
class DebateDemo:
    """Multi-agent debate demo runner."""
    ...
    async def run(self, topic: str) -> dict:
        ...
        # Round 3: Proponent rebuts (optional - can be enabled for longer debates)
        # Round 4: Opponent rebuts
        ...
        # Try to extract verdict from moderator's response
        verdict = "TIE"
        ...
        # Build a human-readable summary for the "response" key
        response_parts = []
        for r in rounds:
...
register_demo(DemoInfo(
    ...
    description="Three agents debate a topic: one argues FOR, one argues AGAINST, and a moderator declares a winner. Showcases multi-agent sequential orchestration with interdependent outputs.",
    icon="🎭",
